...
def main():
    ...
    from model_sigmaGPT import AOGPT, AOGPTConfig
    ...
    ckpt_dir = '/home/ckpt/sigmaGPT/350M'
            ...
            with torch.no_grad():
                ...
                for j, batch in enumerate(eval_iter):
                    for i in range(monte_carlo_steps):
                        _, cur_loss = model(batch['input_ids'].to(device), mode = 'Random')
                        total_loss += cur_loss
                        batch_num += 1
    ...
    #             total_loss += cur_loss
    #             batch_num += 1
    #         if batch_num >= int(16/ddp_world_size)*monte_carlo_steps:
    #             break
